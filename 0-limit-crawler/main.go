...
func Crawl(url string, depth int, wg *sync.WaitGroup, timerChannel <-chan time.Time) {
	defer wg.Done()
	<-timerChannel
	...
	for _, u := range urls {
		...
		// called concurrently
		go Crawl(u, depth-1, wg, timerChannel)
	}
}
...
func main() {
	...
	timerChannel := time.Tick(rateLimit)

	wg.Add(1)
	Crawl("http://golang.org/", 4, &wg, timerChannel)
	wg.Wait()
}
